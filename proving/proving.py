...
def cal_uncerts(t,pdf,std_devs):
    ...
    for j in prange(len(std_devs)):
        u = 0
        std_dev = std_devs[j]
        for i in range(len(t)-len(t[t>=-std_devs[j]]),len(t)-len(t[t>=std_devs[j]])-1):
...
def get_confidence_intervall(coverage_factor):
    ...
    dx = x[1]-x[0]
    u = 0
    for i in range(len(x)-len(x[x>=-std_dev*coverage_factor]),len(x)-len(x[x>=std_dev*coverage_factor])):
...
def calculate_uncertanity(x,**params):
    ...
    if len(params)!=0:
        if 'confidence_intervall' not in params.keys() and 'coverage_factor' not in params.keys():
            confidence_intervall = 0.95
        ...
        for key, value in params.items():
            if key == 'confidence_intervall':
                if 'coverage_factor' in params.keys() and params['coverage_factor'] > 0.0:
            ...
            if key == 'coverage_factor':
                if 'confidence_intervall' in params.keys() and params['confidence_intervall'] > 0.0:
    ...
    w = (max(x)-min(x))/x_bar #Repeatability of datapints as a fraction aka range of values
    dof = n-1
    x_n = x_n = np.array(x)/x_bar #Noralized datapoints as a fraction
    x_t = (np.array(x)-x_bar)/np.std(x,ddof=1) #centerd and scaled the data points
    x_t_b = (np.array(x)-x_bar)/(np.std(x,ddof=1)/np.sqrt(n)) #centerd and scaled the data points - bessel corrected
    ...
    if print_result:
        print(w)
        print(
            "Result = {:.3f}".format(x_bar)+" ± {:.3f}".format(uncertanity_in_unit)
        )
        print(
            "Result = {:.3f}".format(x_bar)+" ± {:.4f}%".format(uncertanity_fraction*100)
        )
